...
export const PairSelector = ({ pair, onChange }: PairSelectorProps) => {
    ...
    const handleChange = (e: SelectChangeEvent<string>) => {
        const selectedPair = tokenPairList.find(item => item.pair = e.target.value)
                    ...
                    tokenPairList.map((pair, index) => (
                        <MenuItem value={pair.pair} key={index}>
                            <Box display="flex" alignItems="center" sx={{ width: '100%' }}>
                                <Image src={pair.logoA} width={30} height={30} alt={pair.tokenA} />
                                <Image src={pair.logoB} width={30} height={30} alt={pair.tokenB} />
                                <Typography sx={{ pl: 4 }}>{pair.symbol}</Typography>
                                <Box sx={{ ml: 'auto' }}>
                                    <Typography sx={{ color: "text.active" }}>{parseFloat(pair.change) >= 0 ? '+' + pair.change : pair.change}%</Typography>
                                </Box>
                            </Box>
                        </MenuItem>
                    ))
